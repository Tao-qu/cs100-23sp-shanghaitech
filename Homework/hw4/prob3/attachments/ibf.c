...
struct brainfuck_state
{
  ...
  size_t *loop_buffer_size; /* The loop buffer's size of each depth. */
  size_t depth;             /* The depth of loops. */
};
...
brainfuck_state_new()
{
  ...
  //// --- FOR PART B ONLY --- Uncomment to start part B ---
  state->loop_buffer_size = calloc(BRAINFUCK_MAX_LOOP_DEPTH, sizeof(size_t));
  state->depth = 0;
  return state;
}
...
//
void brainfuck_execute_loop(struct brainfuck_state *state, char *src)
{
  src++;
  size_t unmatched_depth = 1;
  state->loop_buffer_size[state->depth] = 0;
  while (unmatched_depth)
  {
    state->loop_buffer_size[state->depth]++;
    if (*src == BRAINFUCK_TOKEN_LOOP_START)
    {
      unmatched_depth++;
    }
    else if (*src == BRAINFUCK_TOKEN_LOOP_END)
    {
      unmatched_depth--;
    }
    src++;
                    ...
                    brainfuck_input_handler input_handler, brainfuck_output_handler output_handler)
{
  ...
  while (*src)
  {
    ...
    case BRAINFUCK_TOKEN_LOOP_START:
      brainfuck_execute_loop(state, src);
      if (state->memory_buffer[state->memory_pointer_offset] == 0)
      {
        src += state->loop_buffer_size[state->depth];
      }
      else
      {
        state->depth++;
