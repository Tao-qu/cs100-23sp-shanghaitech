...
public:
     ...
     * 3. Any live cell with more than three live neighbours dies, as if by over-population.
     * 4. Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
     * 5. the living cells have different colors. When a cell is born, its color will be decided by the major color of its three living neighbors.
     *
     * @param current A pointor to a cell to be updated
     * @param neighbors a vector of `CellPointer`, containing all the neighbors of the cell we working on. You need to count the number of living cells in neighbors to determine the next state.
     */
...
public:
     ...
     * 3. Any live cell with more than three live neighbours dies, as if by over-population.
     * 4. Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
     * 5. The living cells are all weighted, meaning that some neighbors might have greater influence.
     *
     * @param current A pointor to a cell to be updated
     * @param neighbors a vector of `CellPointer`, containing all the neighbors of the cell we working on. You need to count the number of living cells in neighbors to determine the next state.
     */
     ...
     * @return int
     */
    virtual int CountAliveNeighbors(const CellNeighbors &neighbors) const override;
...
public:
     ...
     * 3. Any live cell with more than three live neighbours dies, as if by over-population.
     * 4. Any dead cell with exactly three live neighbours becomes a live cell, as if by reproduction.
     * 5. The living cells will not die immediately when overpopulation or underpopulation happens. Instead, the cells 'get older' before the final death.
     *
     * @param current A pointor to a cell to be updated
     * @param neighbors a vector of `CellPointer`, containing all the neighbors of the cell we working on. You need to count the number of living cells in neighbors to determine the next state.
     */
